...
from config import SOURCE_DIR, DOCS_DIR, GRAPH_PATH, CHROMA_PATH
...
def main():
    ...
    print(f"  OK  Found {len(all_files)} source files")
    for f in all_files:
        print(f"      {f.name}")
    ...
    print(f"  OK  Graph: {G.number_of_nodes()} subroutines, {G.number_of_edges()} call edges")
