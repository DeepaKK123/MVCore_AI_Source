...
Usage (standalone test):
    python -m parser.mv_parser path/to/MY.SUBROUTINE.B
"""
...
def parse_mv_file(file_path: str) -> SubroutineInfo:
    """Parse a single MV source file and return structured metadata."""
    path = Path(file_path)
    info = SubroutineInfo(name=path.name, file_path=file_path)
    open_count = 0
    close_count = 0
    ...
    with open(file_path, "r", errors="ignore") as f:
        for lineno, line in enumerate(f, 1):
            ...
            # Skip comment lines (MV BASIC uses * for comments)
...
if __name__ == "__main__":
    import sys, json
    if len(sys.argv) < 2:
        print("Usage: python -m parser.mv_parser <path/to/file.B>")
        sys.exit(1)
    result = parse_mv_file(sys.argv[1])
